                        ...
                        <form id="desactive_form" action="db/desactiverDoc.php" method="post">
                            <div class="card-body">
                                <table class="table table-striped table-hover ">
                                    ...
                                    $id_doc = $_GET["id"];
                                    $qq = mysqli_query($conn,"SELECT * FROM liaison where id_doc = $id_doc");
                                    while($ss = $qq->fetch_assoc()){
                                    $query = ("SELECT * FROM document where  id = ".$ss["id_anx"]);
                                    $result = mysqli_query($conn,$query);
                                    while($obj = $result->fetch_assoc()){
                                        
                                        $sql = mysqli_query($conn,"SELECT * FROM service WHERE valide = '0' and active_type = '0' and id = ".$obj["id_service"]);
                                        if($sql){
